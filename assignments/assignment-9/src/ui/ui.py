...
class UI:
    ...
    def __handle_statistics(self):
        ...
        command_statistics = input("Choose an option: ")

        if not command_statistics:
            print("No command given!")
            return
        try:
            command_statistics = int(command_statistics)
        except:
            print("Invalid command!")
            return
        if not (1 <= command_statistics <= 3):
            print("Invalid command!")
            return

        if command_statistics == 1:
            print("Students and their assignments:")
            pprint(self._services.list_grade())

            assignment_id = input("Choose the assignment id you want to order the students by:")
            try:
                assignment_id = int(assignment_id)
            except Exception as e:
                print(e)
            pprint(self._services.stud_given_assignment_grade(assignment_id))

        elif command_statistics == 2:
            pprint(self._services.list_grade())
            ...
            print("\nStudents that are late handing in their assignments:")
            pprint(self._services.late_assignment(current_week))

        elif command_statistics == 3:
            print("\nSchool situation\n")
            pprint(self._services.school_situation())
